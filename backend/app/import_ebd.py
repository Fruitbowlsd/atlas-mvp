"""CLI fuer den Import des Codelisten-Teils aus "Entscheidungsbaum-Diagramme und
Codelisten fuer die Antwortnachrichten" 4.2 (Issue #50).

Wie bei AHB, MIG und OBIS-Codeliste bewusst ein Kommandozeilenwerkzeug und KEIN
API-Endpunkt: der Import ist ein kuratierter Vorgang, dessen Report gelesen wird.

    cd backend && ./venv/bin/python -m app.import_ebd \\
        --pdf data/regulatory/EBD_und_Codelisten_4_2_20251001.pdf

Die Entscheidungsbaum-Diagramme selbst sind NICHT Gegenstand dieses Imports
(Auftrag Abschnitt 10). Der Report weist sie nur nach Anzahl und Seitenbereich
aus.
"""
...
from .regulatory_extraction_ebd import (
    import_ebd_codelists,
    resolve_active_regulatory_version,
)

_DEFAULT_PDF = "data/regulatory/EBD_und_Codelisten_4_2_20251001.pdf"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Codelisten der Antwortnachrichten (EBD-Dokument) in die "
    ...
    try:
        ...
        report = import_ebd_codelists(db, args.pdf, version.id, reimport=args.reimport)
