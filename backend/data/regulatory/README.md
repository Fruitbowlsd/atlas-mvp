...
Der Import ist idempotent: derselbe Dateihash wird beim zweiten Lauf erkannt und
sauber uebersprungen (`--reimport` erzwingt einen kontrollierten Neuaufbau).

## Tests

Die Tests in `backend/tests/` ueberspringen sich selbst, wenn die PDF-Datei nicht
vorliegt. Ein abweichender Ablageort laesst sich ueber die Umgebungsvariable
`ATLAS_AHB_GAS_11_PDF` setzen.
