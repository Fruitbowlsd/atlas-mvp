...
Verweise am Ende jedes Etappenberichts auf die Ticketnummer, damit der Zusammenhang nachvollziehbar bleibt. Aktuelles Ticket: **#64**, PR der Etappen 0–7: **#65** (bereits nach `main` gemergt, Merge-Commit `f3594f9`).

Der gesamte Auftrag (Etappe 0–8) sollte ursprünglich bis zum bestätigten Übergabebericht (Etappe 8) in einem einzigen PR/Branch bleiben — kein Zwischen-Merge nach `main` vor Abschluss, siehe Anhang. **Revidiert (2026-09-18, nach Abschluss von Etappe 7):** Diese Regel galt vor allem, weil Etappe 7 (Konsolidierung) zweimal frühere Etappen-Befunde revidiert hat (O-16, O-18) und die Datenbasis bis dahin noch in Bewegung war. Mit dem final konsolidierten Regulatory-State-Objekt aus Etappe 7 ist die Datenbasis eingefroren; Etappe 8 fasst nur noch zusammen und ändert sie nicht mehr. **Umgesetzt (2026-09-18):** O-20 ist entschieden (siehe 11g), PR #65 (Etappen 0–7) ist nach `main` gemergt. Etappe 8 läuft in einer eigenen Worktree (Branch `feature/regulatory-state-etappe8`, von `main` abgezweigt) statt im Hauptarbeitsverzeichnis, weil dort parallel an einem eigenen, unabhängigen Thema (`fix/sidebar-claim`) gearbeitet wird und diese Arbeit nicht gestört werden soll.
...
**Prüfpunkt vor tatsächlicher Umsetzung (ergänzt 2026-09-18):** Dieser Auftrag bleibt laut 15a bewusst auf Ebene von Recherche-Artefakten (Markdown/JSON) — es wird keine Schemaänderung an der produktiven Atlas-Datenbank vorgenommen. Bevor die hier vorgeschlagene Erweiterung (`RegulatoryVersion.regulatory_state_id`, neue `Regulatory State`-Tabelle) tatsächlich als Migration umgesetzt wird — also nach Abschluss dieses Auftrags, im Rahmen von Abschnitt 12/13 —, muss die reale Postgres-Schema im Repo (Migrationen und ORM-Modelle unter `backend/`) direkt eingesehen werden: tatsächliche Tabellen- und Spaltennamen, Datentypen, Migrations-Tooling und bestehende Konventionen. Der Vorschlag oben ist konzeptionell gegen den Bestand geprüft (siehe `vorschlag_regulatory_state_modell.md`), ersetzt aber nicht die Prüfung gegen das reale Schema zum Zeitpunkt der Umsetzung.
...
### 11g. Übertragungsweg / AS4 / API / XML: Ergebnisse, Rollenergänzungen, O-4 korrigiert, O-19 gelöst, O-20 entschieden (bestätigt 2026-09-18, aus Etappe 6, final entschieden in Etappe 7)
...
**O-4 (API-Webdienste), Grundbefund weiterhin gültig:** Alle drei API-Versionen stammen aus verbindlichen Mitteilungen — „Steuerungshandlungen 1.0.0" (Anlage Mitteilung 36, bytegleich mit BDEW-PDF 7313), „MaLo-ID 1.0.0" (Anlage Mitteilung 43, bytegleich mit 7314), „Verzeichnisdienst API 1.0" (Anlage Mitteilung 46). PID 4.0 führt die Webdienste unter denselben Namen. Release 2.0.0 ist nur ein Konsultations-Release und gilt am Stichtag nicht. Kriterium A (Mitteilungsbezug) ist damit für alle drei Dokumente unangetastet erfüllt. **Zur zusätzlich behaupteten „Kanalwechsel"-Einordnung (PDF → GitHub-Release, angeblich ohne inhaltliche Änderung) siehe die Korrektur unten (O-20) — diese Einordnung war zu stark und ist zurückgenommen.**
...
**Rollenergänzungen in `rollendefinition.md`, Regeln R-6a–c (bestätigt 2026-09-18, R-6b am 18.09.2026 korrigiert):**
* **R-6a** — Existiert kein PDF, ist die XSD- bzw. YAML-Datei selbst das maßgebliche Dokument (siehe auch Abschnitt 7).
* **R-6b** — Wechselt bei gleicher Version nur der Veröffentlichungsweg, wird die alte Fassung `ersetzt` — nur mit doppeltem Beleg: Inhalt nachweislich unverändert **und** der neue Weg regulatorisch geregelt. **Bei den drei API-Webdiensten ist diese Regel aktuell nicht erfüllt** (siehe O-20 unten) — `derzeit kein erfüllter Anwendungsfall`.
* **R-6c** — Existiert für ein Dokument keine Versionsnummer (nur eine Änderungshistorie), ist die jüngste Veröffentlichung maßgeblich — dieselbe "neuester Stand gilt"-Logik wie in 6/6a, hier für den versionslosen Fall.
...
**O-19(b) beantwortet (bestätigt 2026-09-18, aus Etappe 7):** Die Regelungen zum Verzeichnisdienst 1.1 setzen die nie verbindlich übernommene API-Version 1.1 inhaltlich nicht voraus. Eigenständiger Zusatzbefund dabei, der nicht in der Schließung von O-19(b) untergehen soll: Kapitel 4.6 dieser Regelungen verlangt eine Ankündigung bei Einstellung des Betriebs — die gültige API 1.0.0 hat dafür kein eigenes Schnittstellenfeld. Das ist eine Lücke zwischen Regelungstext und Schnittstellenimplementierung und gehört als eigener Hinweis in den Übergabebericht (Etappe 8), getrennt von der O-19(b)-Antwort selbst.

**O-20 entschieden (bestätigt 18.09.2026, Commit `96a3371`):** Bei der O-19(a)-Prüfung zwei Auffälligkeiten in den maßgeblichen Dateien selbst entdeckt: (1) Die maßgebliche Web-API-Datei enthält noch einen Platzhalter „TODO: Referenz auf AsyncAPI-Spec hinzufügen" — die übrigen drei Belege für O-19(a) tragen davon unabhängig. (2) Alle drei API-Dokumente (nicht nur Verzeichnisdienst) verweisen für die vorige Fassung auf SwaggerHub-Spezifikationen, die dort inzwischen gelöscht sind (HTTP 404); auch das Internet Archive hat keinen Inhalt gespeichert — ein Direktvergleich ist also nicht möglich. Zusätzlich stellte sich heraus: Die drei alten BDEW-PDFs (7313/7314/7650) sind reine Linkblätter (rund 1.100 Zeichen, keine Pfade oder Schemas) ohne eigenen Spezifikationsinhalt — der in Etappe 6 herangezogene Bytevergleich betraf nur PDF gegen PDF (BNetzA ↔ BDEW), nicht die Spezifikation selbst. Die Kanalwechsel-Regel **R-6b** verlangt einen *nachweislichen* Beleg für „Inhalt unverändert" — der liegt bei keinem der drei Dokumente vor, nur eine Herausgebererklärung in der jeweiligen Release-Notiz.

**Entscheidung:** Alle drei PDFs (bdew:7313, 7314, 7650) kehren von `ersetzt` auf `ergänzend` zurück; die OpenAPI-/YAML-Dateien bleiben unverändert `maßgeblich`. R-6b bleibt als Regel gültig, hat aber aktuell keinen erfüllten Anwendungsfall — das wird als offene Beobachtung in den Übergabebericht (Etappe 8) übernommen, nicht als gelöster Fall. Der ursprüngliche „O-4 gelöst"-Befund aus Etappe 6 ist entsprechend korrigiert: Kriterium A bleibt für alle drei Dokumente unangetastet gültig, nur die zusätzliche „Kanalwechsel"-Einordnung war zu stark und ist zurückgenommen (siehe Etappe-6-Eintrag im Anhang). Rollenverteilung im finalen Regulatory-State-Objekt entsprechend angepasst: `ergänzend` 99→102, `ersetzt` 32→29 (86 maßgebliche Fassungen unverändert, K-1 unverändert). Damit ist O-20 abschließend entschieden, keine offene Rückfrage mehr.

### 11h. Etappe 7 — Konsolidierung: Endergebnis, Sparten-Korrekturen, K-1/P-1 final (Stand 2026-09-18, vollständig abgeschlossen)

**Vollständigkeitsprüfung gegen BDEW:** Am Stichtag sind 231 Einträge in 90 Themen gültig. 83 von 90 Themen sind durch Datensätze vollständig abgedeckt; die übrigen 7 entsprechen genau den in Etappe 4 mit Negativbeleg ausgeschlossenen Dokumenten (siehe 11e). Gegenprobe: kein gültiger BDEW-Eintrag ohne zugehörige Fassung, keine Versionsabweichung, kein Eintrag mit Gültigkeit nach dem Stichtag.

**Regulatory-State-Objekt final** (`etappe7_regulatory_state.json`): 85 Dokumente, 402 Fassungen, 86 maßgebliche Fassungen (86 statt 85, weil die Verzeichnisdienst-API wegen Schema v0.9 zwei gleichrangige maßgebliche Fassungen hat, siehe 11g). Rollenverteilung final (nach O-20-Entscheidung): `ergänzend` 102, `ersetzt` 29, `nicht_verbindlich` 33, `informativ` 152, `maßgeblich` 86.

**Sparten-Korrektur (Nachbesserung eines Etappe-4-Fehlers, bestätigt 2026-09-18):** Etappe 4 hatte alle zehn Querschnittsdokumente pauschal auf „beide" gesetzt, mit der ungeprüften Begründung „Referenzen aus Dokumenten beider Sparten". Die in Etappe 7 nachgeholte Referenzsuche über alle maßgeblichen Fassungen widerlegt das für drei Fälle:
* Zeitreihentypen — nur aus UTILMD MIG Strom referenziert → korrigiert auf Strom
* Lokationsbündelstrukturen — nur aus UTILMD AHB Strom referenziert → korrigiert auf Strom
* Standardlastprofile nach TU München — nur aus UTILMD MIG Gas referenziert → korrigiert auf Gas

Verwendungszwecke und Artikelnummern bleiben „beide", aber ausdrücklich nur indirekt belegt, nicht gleichrangig mit den in 11d evidenzbasiert belegten 13 Nachrichtentypen. Ändert nichts an der Versionsauswahl, nur an der Sparten-Kennzeichnung.

**Kernbefund K-1, Endstand:** 38 von 85 Dokumenten (45 %) haben bei der maßgeblichen Fassung keinen Mitteilungsbezug — ausnahmslos bei UTILMD, PID und EBD. Für den Übergabebericht (Etappe 8) wichtig ist die strukturelle Erklärung, nicht nur die Prozentzahl: Bei diesen drei Dokumentgruppen ist die maßgebliche Fassung strukturell immer ein Fehlerkorrekturstand, der nicht separat als eigene Mitteilungsanlage geführt wird. Der fehlende Mitteilungsbezug ist damit eine Eigenschaft des Fehlerkorrektur-Mechanismus selbst, kein Zufallsbefund — das gehört so benannt in den Bericht.

**Widersprüche BNetzA/BDEW:** keine. Die einzige Fragebogen-Markierung „Widerspruch: ja" (Frage 8) betrifft O-14 (Bezeichnungsabweichung PID/DVGW), kein Versionswiderspruch.

**Konsultationsfassungen (Frage 7):** 25 Datensätze haben einen Nachfolger in Konsultation. Das deckt sich als Cross-Check exakt mit Mitteilung 57 (23 Dokumente) plus Release 2.0.0 für die zwei Strom-API-Webdienste (Steuerungshandlungen, MaLo-ID).

**O-19(b) und O-20:** beide entschieden/beantwortet, siehe 11g. Keine offene Rückfrage mehr aus Etappe 7.

**P-1 (neu, Produktentscheidung, analog zum Folgepunkt in 9b):** Nur RV 2 (Gas) verweist auf den Regulatory State 01.10.2026, obwohl der State 81 Dokumente mit Strom-Bezug enthält. Bleibt wie der Folgepunkt aus 9b offen für die Produktentscheidung, nicht Teil dieses Rechercheauftrags.

**Status:** Etappe 7 ist damit vollständig abgeschlossen und bestätigt. Die Datenbasis ist eingefroren — Etappe 8 fasst nur noch zusammen, ändert sie nicht mehr.
...
Verbindliche Reihenfolge für diesen Auftrag, geführt als Checkliste im in Abschnitt 0 angelegten Ticket (**#64**). Jede Etappe endet mit einem kurzen Zwischenbericht (siehe Abschnitt 13a) als Ticket-Kommentar und, sofern der Projekt-Workflow das vorsieht, einem eigenen Commit/PR-Zwischenstand — nicht erst am Ende der gesamten Recherche. Die Etappen 0–7 liefen in einem einzigen PR (**#65**, bereits nach `main` gemergt, Merge-Commit `f3594f9`). Etappe 8 läuft jetzt in einer eigenen Worktree (Branch `feature/regulatory-state-etappe8`, von `main` abgezweigt) mit eigenem PR.
...
*Status: abgeschlossen und bestätigt (Commit 4ac4075), Nachkorrektur am 18.09.2026 umgesetzt (Commit 96a3371).* 37 Datensätze mit 149 Fassungen; alle 85 Datensätze der Etappen 1–6 gegen Schema v0.8 validiert. O-4 (API-Webdienste) zunächst als vollständig gelöst gemeldet (siehe 11g) — diese Einordnung war zu stark: Die spätere Linkblatt-Entdeckung zeigt, dass die zusätzliche „Kanalwechsel"-Bewertung nach R-6b für alle drei API-Dokumente auf unzureichendem Beleg beruhte und am 18.09.2026 zurückgenommen wurde (die drei BDEW-PDFs sind `ergänzend` statt `ersetzt`); Kriterium A (Mitteilungsbezug) bleibt davon unberührt gültig. Drei Rollenregeln R-6a–c und Schema v0.8 bestätigt. Dokument-Zuschnitt der API-Webdienste bestätigt (zwei Dokumente für Steuerungshandlungen/MaLo-ID, eines für Verzeichnisdienst). Zweiter Anwendungsfall der Kriterium-C-Erweiterung (Verzeichnisdienst, versionslos) eigenständig geprüft. Kernbefund K-1 durch diese Etappe deutlich verschärft: 18 von 37 maßgeblichen Fassungen ohne Mitteilungsbezug (Zwischenstand, siehe 11h für den finalen Wert 38/85). Ticket #64 kommentiert und abgehakt, Draft-PR-Beschreibung um Etappen 3–6 nachgetragen. Bericht: `etappe6_uebertragungsweg_api_xml.md`.
...
*Status: abgeschlossen und bestätigt (Commit cef537e, Nachtrag 96a3371 vom 18.09.2026).* Mitteilung-46/48-Chronologie bestätigt (48 nach 46; Fall „konsultiert, nicht übernommen" wie REQOTE MIG 1.3d, siehe 11e/11g). O-19(a) gelöst: Web-API und WebSocket-API sind normativ untrennbar, bleiben ein Dokument; Schema v0.9 (Felder `bestandteil_gruppe`/`bestandteil`) erlaubt dafür mehrere gleichrangige maßgebliche Fassungen je Dokumentversion (siehe 11g). `aufnahmeregel.md` und `rollendefinition.md` entsprechend angepasst. Regulatory-State-Objekt final: 85 Dokumente, 402 Fassungen, 86 maßgebliche Fassungen (`etappe7_regulatory_state.json`). Vollständigkeit gegen BDEW geprüft (83/90 Themen abgedeckt, Rest = Etappe-4-Ausschlüsse), keine Versionsabweichung, keine Lücke. Sparten-Korrektur für drei Etappe-4-Dokumente (Zeitreihentypen, Lokationsbündelstrukturen → Strom; TU München → Gas), bestätigt 18.09.2026, siehe 11h. K-1 im Endstand: 38/85 (45 %), strukturell erklärt (Fehlerkorrekturstände ohne eigene Mitteilungsanlage bei UTILMD/PID/EBD), siehe 11h. O-19(b) beantwortet und bestätigt, mit eigenständigem Zusatzbefund zur Kapitel-4.6-Lücke (siehe 11g). O-20 entschieden (18.09.2026): Kanalwechsel-Regel R-6b bei keinem der drei API-Dokumente erfüllt, die drei BDEW-PDFs sind `ergänzend` statt `ersetzt`, R-6b bleibt gültig ohne aktuell erfüllten Anwendungsfall — offene Beobachtung für Etappe 8. Neuer Punkt P-1 (fehlende Strom-RV-Verknüpfung, siehe 11h). Datenbasis eingefroren, keine offene Rückfrage mehr. Bericht: `etappe7_konsolidierung.md`, JSON: `etappe7_regulatory_state.json`.

**Etappe 8 — Übergabebericht**
Zusammenfassender Bericht an den Menschen: was ist vollständig und belegt, was ist als `relevanz_unklar` markiert, was wurde bewusst nicht vertieft (mit Begründung), welche offenen Rückfragen bestehen. Muss die Kernbefunde **K-1 (Mitteilungsbezug-Lücke, finaler Stand aus Etappe 7: 38 von 85 Dokumenten / 45 %, strukturell erklärt durch den Fehlerkorrektur-Mechanismus bei UTILMD/PID/EBD — nicht die Etappe-6-Zwischenzahl 18/37 zitieren, siehe 11h)** und **K-2 (dritte Quelle bei Gas, Aufnahmeregel)** prominent enthalten, ebenso P-1 (fehlende Strom-RV-Verknüpfung, 11h/9b), O-20 als offene Beobachtung (R-6b ohne erfüllten Anwendungsfall, 11g) und den eigenständigen Zusatzbefund zu Kapitel 4.6 / API 1.0.0 (11g). O-20 ist bereits geklärt (siehe 11g) — kein Blocker mehr für den Start dieser Etappe. Arbeitsumgebung: eigene Worktree, Branch `feature/regulatory-state-etappe8`, von `main` abgezweigt (siehe Abschnitt 0). Ergebnis als PR gegen das Ticket aus Abschnitt 0 einreichen. Kein Übergang zur fachlichen Analyse (Abschnitt 12/16) ohne Bestätigung dieses Berichts.
