<!-- Repo-Notiz (nicht Teil des Auftragstexts): Wortlaut des Claude-Projektdokuments,
     Fassung vom 18.09.2026 (Stand nach Etappe 6), an diesem Tag in die Arbeitssitzung
     übergeben und hier unverändert abgelegt. Ersetzt die Fassung vom 18.09.2026 (Stand nach O-18). Abweichungen zwischen
...
**Zweiter Anwendungsfall der Erweiterung, eigenständig neu geprüft (bestätigt 2026-09-18, aus Etappe 6):** Die Regelungen zum Verzeichnisdienst referenzieren ohne Version die „Verzeichnisdienst API" — Eindeutigkeit wurde nicht vom TU-München-Fall übernommen, sondern eigenständig geprüft (wie von der Regel gefordert). Dabei aufgetaucht: eine Version 1.1, über Mitteilung 48 konsultiert, aber nie verbindlich veröffentlicht. Offene Rückfrage zur Chronologie und Einordnung siehe 11g.
...
### 11g. Übertragungsweg / AS4 / API / XML: Ergebnisse, Rollenergänzungen, O-4 gelöst, O-19 neu (bestätigt 2026-09-18, aus Etappe 6)
...
**Offener Punkt O-19 (neu):**
* **(a) Modellfrage, bewusst nicht von Claude Code entschieden:** Das Verzeichnisdienst-Release besteht aus zwei gleichrangigen Dateien (Web-API und WebSocket-API); das Schema erlaubt aber nur eine maßgebliche Fassung je Dokumentversion. Vorläufig steht die WebSocket-API als `ergänzend`. **Empfohlene Lösung (bestätigt 2026-09-18):** Falls Web-API und WebSocket-API inhaltlich eigenständige Spezifikationen sind (unterschiedliche Schnittstellen, nicht wechselseitig unverständlich ohne die andere), werden sie — analog zum Zuschnitt Steuerungshandlungen/MaLo-ID oben — als zwei separate Dokumente geführt statt als eine Dokumentversion mit erzwungener Hierarchie. Nur falls sie untrennbar zusammengehören, ist eine Schemaerweiterung (mehrere gleichrangige maßgebliche Fassungen je Dokumentversion) zu diskutieren.
* **(b) Ungeprüft, niedrige Priorität:** Ob die Regelungen zum Verzeichnisdienst 1.1 inhaltlich die API 1.0 voraussetzen, obwohl 1.1 nie verbindlich veröffentlicht wurde. Bleibt offen, da 1.1 ohnehin nicht verbindlich ist.

**Offene Rückfrage zur Kriterium-C-Anwendung (siehe 11e):** Chronologie von Mitteilung 46 vs. Mitteilung 48 zu bestätigen — falls 48 vor 46 lag, ist die Einordnung von Verzeichnisdienst-API 1.1 als „früherer Stand" korrekt; falls 48 nach 46 lag, handelt es sich eher um einen Fall wie Mitteilung 57 (zukünftige, nicht verbindliche Konsultation) und sollte entsprechend im Datensatz vermerkt werden. Ändert nichts am Ergebnis für den Stichtag, nur an der Begründung.
